...
def chooseNewestFile(folder):
    ...
    for fileName in fileList:
        ...
        for i in range(len(fileDateTime)):
            # As you're comparing sections of date/time, if value is same, move onto next section
            if int(fileDateTime[i]) == int(newestFileDateTime[i]):
                continue
            # As you're comparing sections of date/time, if value is less, then file is older
            elif int(fileDateTime[i]) < int(newestFileDateTime[i]):
                break
            # As you're comparing sections of date/time, if value is greater, then file is newer, assign current file as new file
            elif int(fileDateTime[i]) > int(newestFileDateTime[i]):
                newestFile = fileName
                break
